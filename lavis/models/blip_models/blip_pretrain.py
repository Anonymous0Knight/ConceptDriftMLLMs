...
def tSP_Metric(x, y, kappa=16):
    return 2 / (1  + kappa * (1 - x@y))
...
class BlipPretrain(BlipBase, SharedQueueMixin, MomentumDistilationMixin):
    ...
    PRETRAINED_MODEL_CONFIG_DICT = {
        "base":  "configs/models/blip/blip_pretrain_base.yaml",
        "large": "configs/models/blip/blip_pretrain_large.yaml",
    }
    ...
    def forward(self, samples):
        ...
        with torch.no_grad():
            ...
            sim_i2t_m = tSP_Metric(image_feat_m, text_feat_all) / self.temp
            sim_t2i_m = tSP_Metric(text_feat_m , image_feat_all) / self.temp
        ...
        sim_i2t = tSP_Metric(image_feat,  text_feat_all)  / self.temp
        sim_t2i = tSP_Metric(text_feat , image_feat_all)  / self.temp
